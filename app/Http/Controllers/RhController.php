...
class RhController extends Controller
{
    ...
    public function exportarExcelFiltradas(Request $request)
    {
        ...
        $nomeArquivo = "escalas_{$ano}" . ($mes ? "_{$meses[$mes]}" : "") . ".xlsx";
        
        $spreadsheet = new Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();
        $sheet->setTitle('Escalas');
        ...
        foreach ($escalas as $escala) {
            ...
            $sheet->setCellValue("A{$row}", "{$unidadeNome} - {$mesNome}/{$escala->ano}");
            $sheet->getStyle("A{$row}:F{$row}")->applyFromArray($sectionStyle);
            $row++;
            
            $headers = ['Núm.', 'Matrícula', 'Nome do Servidor', 'Horas', 'Módulo', 'Dias'];
            ...
            foreach ($alocacoesAgrupadas as $a) {
                ...
                $sheet->setCellValue("A{$row}", str_pad($num, 3, '0', STR_PAD_LEFT));
                $sheet->setCellValue("B{$row}", $a['matricula']);
                $sheet->setCellValue("C{$row}", $a['servidor_nome']);
                $sheet->setCellValue("D{$row}", $a['horas']);
                $sheet->setCellValue("E{$row}", $a['modulo_nome']);
                $sheet->setCellValue("F{$row}", $diasStr);
            ...
            if ($row > $dataStartRow) {
                $sheet->getStyle("A{$dataStartRow}:F" . ($row - 1))->applyFromArray($dataStyle);
            }
            
            $sheet->mergeCells("A{$row}:C{$row}");
            $sheet->setCellValue("A{$row}", 'Total da Escala:');
            $sheet->setCellValue("D{$row}", $totalHorasEscala);
        ...
        $sheet->setCellValue("A{$row}", 'Total de Escalas:');
        $sheet->setCellValue("B{$row}", $escalas->count());
        $sheet->getStyle("A{$row}:B{$row}")->applyFromArray($dataStyle);
        $sheet->getStyle("B{$row}")->getFont()->setBold(true);
        $row++;
        
        $sheet->setCellValue("A{$row}", 'Total Geral de Horas:');
        $sheet->setCellValue("B{$row}", $totalGeralHoras);
        $sheet->getStyle("A{$row}:B{$row}")->applyFromArray($dataStyle);
        $sheet->getStyle("B{$row}")->getFont()->setBold(true);
    ...
    public function exportarExcel($id)
    {
        ...
        $mesNome = $meses[$escala->mes];
        $ano = $escala->ano;
        $nomeArquivo = "escala_{$escala->id}_{$mesNome}_{$ano}.xlsx";
        ...
        $sheet->setCellValue('A1', $unidadeNome);
        $sheet->getStyle('A1')->getFont()->setBold(true)->setSize(14);
        $sheet->getStyle('A1')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
        
        $sheet->mergeCells('A2:F2');
        $sheet->setCellValue('A2', "Escala Extraordinária - {$mesNome}/{$ano}");
        ...
        if ($lastDataRow >= $headerRow + 1) {
            $sheet->getStyle("A" . ($headerRow + 1) . ":F{$lastDataRow}")->applyFromArray($dataStyle);
        }
        
        $row += 1;
        $sheet->mergeCells("A{$row}:C{$row}");
        $sheet->setCellValue("A{$row}", 'Autorização de quantitativo a maior é realizado pela SGP');
        $sheet->setCellValue("D{$row}", $totalHoras);
